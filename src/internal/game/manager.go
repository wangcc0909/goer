	...
	ctrl:
		for {
			...
			case uid := <-m.chReset:
				...
				if p.session != nil {
					logger.Infof("玩家正在游戏中,不能重置:%d", uid)
					return
				}
				p.desk = nil
				logger.Infof("玩家重置,UID=%d", uid)
...
func (m *Manager) Login(s *session.Session, req *protocol.LoginToGameServerRequest) error {
	...
	if p, ok := m.player(uid); !ok {
		...
		m.group.Leave(s)
		//重置之前的session
		if preSession := p.session; preSession != nil && preSession != s {
			//如果之前房间存在  则退出来
			if p, err := playerWithSession(preSession); err != nil && p != nil && p.desk != nil && p.desk.group != nil {
				p.desk.group.Leave(s)
			}
			preSession.Clear()
			preSession.Close()
		}
...
func (m *Manager) CheckOrder(s *session.Session, msg *protocol.CheckOrderRequest) error {
	log.Infof("%+v", msg)
...
func (m *Manager) offline(uid int64) {
	...
	log.Infof("玩家: %d从列表中删除,剩余: %d", uid, len(m.players))
}
