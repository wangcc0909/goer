...
const (
	...
	deskPlayerNumEnoughMessage = "您加入的房间已经满人,请确认房间号后再次输入"
	versionExpireMessage       = "你当前的游戏版本过老,请更新客户端,地址为:http://fir.im/leck"
...
var (
	...
	reentryDesk          = &protocol.CreateDeskResponse{Code: 30003, Error: "您当前正在房间中"}
...
func (manager *DeskManager) UnCompleteDesk(s *session.Session, _ []byte) error {
	...
	if p.desk == nil {
		p.logger.Debug("DeskManager.UnCompleteDesk: 玩家不再房间内")
...
func (manager *DeskManager) ReEnter(s *session.Session, msg *protocol.ReEnterDeskRequest) error {
	...
	if p.desk == nil {
		p.logger.Debugf("玩家没有未完成房间,但是发送了重新进入请求: 请求房号: %s", msg.DeskNo)
		return nil
	}
	d := p.desk
...
func (manager *DeskManager) Resume(s *session.Session, _ []byte) error {
	...
	//有玩家切出游戏 切回来时发现有人申请解散,则刷新最新的解散状态
...
func (manager *DeskManager) DingQue(s *session.Session, msg *protocol.Dingque) error {
	...
	que := msg.Que
	if que < 1 {
		return fmt.Errorf("玩家定缺麻将不能未0,实际=%d", que)
	}
	d := p.desk
	if d == nil {
		p.logger.Debug("玩家不再房间内")
...
func (manager *DeskManager) Join(s *session.Session, data *protocol.JoinDeskRequest) error {
	...
	if err := d.playerJoin(s, false); err != nil {
		d.logger.Errorf("玩家加入房间失败,UID=%d, Error=%d", s.UID(), err.Error())
	}
	return s.Response(&protocol.JoinDeskResponse{
		TableInfo: protocol.TableInfo{
...
//有玩家请求解散房间
func (manager *DeskManager) Dissolve(s *session.Session, data *protocol.DissolveStatusRequest) error {
	logger.Debugf("DeskManager.Dissolve: %+v", data)
	p, err := playerWithSession(s)
	if err != nil {
		return err
	}
	d := p.desk
	if d == nil || d.isDestroy() {
		p.logger.Infof("申请解散,但是房间为空或已解散")
		return s.Push("onDissolveSuccess", protocol.EmptyMessage)
	}
...
func (manager *DeskManager) VoiceMessage(s *session.Session, msg []byte) error {
	...
	if d != nil && d.group != nil {
		d.group.Broadcast("onVoiceMessage", msg)
	}
	return nil
}
