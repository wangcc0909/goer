...
type Desk struct {
	...
	paoPlayer     int64                        //炮手
	isMarkerSet   bool                         //当局第一次设置庄家
	dissolve      *dissolveContext             //解除相关状态(改状态只可能是逻辑线程更新,不会并发)
	prepare       *prepareContext              //准备相关状态
	dice          *dice                        //骰子
	lastTileID    int                          //最后一张出牌
	lastChuPaiUid int64                        //最后一个出牌的玩家
	lastHintUid   int64                        //最后一个接到提示的玩家
	latestEnter   *protocol.PlayerEnterDesk    //最新进入的状态
	logger        *log.Entry
}
...
func (d *Desk) desc(detail bool) string {
	...
	if detail {
		if opts.Pinghu && opts.Mode == ModeTrios { //三人模式
...
func (d *Desk) start() {
	...
	var (
		totalPlayerCount = d.totalPlayerCount()
		totalTileCount   = d.totalTileCount()
	)
	//第一局随机庄,以后每局的庄都是上一局第一个和牌者或点双响炮者
	if d.isFirstRound {
...
func (d *Desk) qiPaiFinished(uid int64) error {
	...
	if d.opts.Mode == ModeTrios {
		...
		for _, p := range d.players {
			...
			p.session.Push("onDingQueHint", protocol.Dingque{Que: que})
		}
	}
	return nil
}
...
func (d *Desk) chiPai(tileId int) int {
		...
		for _, op := range ops {
			...
			case protocol.OptypeHu:
				...
				d.logger.Debugf("检查过手胡,CurTurn=%d CheckTurn=%d PreTurn=%d NextTurn=%d CurTileIndex=%d CurTileId=%d",
					d.curTurn, checkTurn, preTurn, nextTurn, curTile.Index, curTile.Id)
				//过手胡规则处理
				if chuPlayer.ctx.PrevOp == protocol.OptypeGang || d.noMoreTile() || preTurn == checkTurn {
					...
					//如果当前打牌和上家打牌一样.则不能点炮
					//从nextTurn的下一家一直检查到打牌人的上一家,如果最后一张打牌和当前打牌的index一样
					//则不允许和牌
					...
					for i := nextTurn; i <= preTurn; i++ {
						...
						d.logger.Debugf("-->检查过手胡: CurTurn=%d CheckTurn=%d PreTurn=%d NextTurn=%d Index=%d CurTileIndex=%d CurTileId=%d",
							d.curTurn, checkTurn, preTurn, nextTurn, i, curTile.Index, id)
	...
	//todo 如果一炮双响的时候 先通知玩家还可以选择碰杠/玩家选择碰杠,第二个玩家选择胡牌,需要仔细推敲
	for winTurn := range winOps {
		player := d.players[winTurn]
		uid := player.Uid()

		//选择了过
		optType := player.hu(tileId, true)
		if optType == deskDissolved {
			return deskDissolved
		}
		if optType != protocol.OptypeHu {
			continue
		}
		paoCount++
		...
		d.logger.Debugf("玩家胡牌: UID=%d 胡=%+v 手牌=%+v 碰杠=%+v", uid,
			mahjong.TileFromID(tileId), player.handTiles(), player.pgTiles())
	...
	isPeng, isGang, isDissovle := player.pengOrGang(tileId, pgOps.op, winOps[pgOps.turn])
	if isDissovle {
		return deskDissolved
	}
...
func (d *Desk) qiangGang(tid int) (bool, bool) {
	...
	for {
		...
		//不能抢杠和
		if !nextPlayer.checkHu(tid, false) {
			continue
		}
		...
		d.logger.Debugf("玩家胡牌: UID=%d, 胡=%+v 手牌=%d 碰杠=%+v",
...
func (d *Desk) roundOverHelper() *protocol.RoundOverStats {
	...
	//无叫玩家需要赔付给有叫玩家,并且无叫的玩家之前的所有刮风下雨积分全部清零
	...
	if len(pei) > 0 {
		...
		//给有叫的人赔叫 , 按最大的番数赔
		//有可能剩下两个人都没叫,不需要赔其他人,只需要清空刮风下雨的积分即可
		for uid := range ting {
...
func (d *Desk) roundOver() {
	...
	d.logger.Debugf("本轮游戏结束,状态=%s 结算状态=%#v", status.String(), stats)
	//round over
	...
	/**
	walk around
	1.A切后台,开始定时器,B,C开始弹出"申请解散对话框"
	2.B申请解散
	3.B,C Ready(正常情况此处在Ready前,有一个C,允许\拒绝的消息)
...
func (d *Desk) finalSettlement(isNormalFinished bool, ge *protocol.RoundOverStats) {
	...
	f := func() []protocol.MatchStats {
		...
		for _, p := range d.players {
			...
			if stats[uid] != nil {
				ms.TotalScore = stats[uid].TotalScore
				ms.ZiMoNum = stats[uid].ZiMoNum
				ms.HuNum = stats[uid].HuNum
				ms.PaoNum = stats[uid].PaoNum
				ms.AnGangNum = stats[uid].AnGangNum
				ms.MingGangNum = stats[uid].MingGangNum
			}
			ms.Uid = uid
			ms.Account = p.name
			mss = append(mss, ms)
...
func (d *Desk) destroy() {
	...
	for i := range d.players {
		p := d.players[i]
		d.logger.Debugf("销毁房间,清楚玩家%d数据", p.Uid())
		p.reset()
		p.desk = nil
		p.score = 1000
		p.turn = 0
...
func (d *Desk) scoreChangeHelper(winner int64, losers []Loser, typ ScoreChangeType, tileID int) {
	...
	for _, loser := range losers {
		d.scoreChangeForUid(winner, &scoreChangeInfo{
			score:  loser.score, //赢了多少分(正分)
			uid:    loser.uid,   //谁输得
			typ:    typ,
			tileID: tileID,
		})
		d.scoreChangeForUid(loser.uid, &scoreChangeInfo{
			score:  loser.score, //输了多少分(负分)
			uid:    winner,      //谁赢的
			typ:    typ,
			tileID: tileID,
		})
...
func (d *Desk) onPlayerExit(s *session.Session, isDisconnect bool) {
	...
	if isDisconnect {
		...
		var resetPlayers []*Player
		for _, p := range d.players {
			if p.Uid() != uid {
				resetPlayers = append(resetPlayers, p)
			} else {
				p.reset()
				p.desk = nil
				p.score = 1000
		...
		d.players = resetPlayers
	}

	//如果桌子上已无玩家 destroy it
	if d.creator == uid && !isDisconnect {
		d.logger.Info("所有玩家下线或者房主主动解散房间")
...
func (d *Desk) applyDissolve(uid int64) {
	...
	d.group.Broadcast("onDissolveAgreement", &protocol.DissolveResponse{
		...
		ResetTime:      applyDissolveRestTime,
	})
}

//收集本桌玩家的解散状态
func (d *Desk) collectDissolveStatus() []protocol.DissolveStatusItem {
	var list []protocol.DissolveStatusItem
	for i, p := range d.players {
		uid := p.Uid()
		status := Waiting
...
func (d *Desk) onPlayerReJoin(s *session.Session) error {
	...
	st := d.status()
	if st != constant.DeskStatusCreate && st != constant.DeskStatusCleaned && st != constant.DeskStatusInterruption {
	...
	if d.dissolve.isDissolving() {
		...
		agreement := &protocol.DissolveResponse{
			DissolveStatus: list,
			ResetTime:      d.dissolve.restTime,
		}
		uid := s.UID()
		//如果断线钱已经同意解散,下次上线则不需要确认解散
