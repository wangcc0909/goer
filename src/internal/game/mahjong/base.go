...
const (
	PingHu   = iota
	Pongpong //碰碰胡 由4副刻子(或杠),将牌组成胡牌
	QiDui    // 七对:七个对子组成的胡牌
	LongQiDui
	ShuangLongQiDui
	HaoHuaLongQiDui
	QYS          //清一色: 由一种花色的牌组成的胡牌
	HaiDiLao     //海底捞:抓桌面上最后一张牌的人正好能胡
...
func Multiple(ctx *Context, onHand, pongkong Indexes) int {
	...
	if gangCount > 0 {
		println("===>", "杠", "+1")
		ctx.Desc = append(ctx.Desc, fmt.Sprintf("跟×%d", gangCount))
		multiple += gangCount
	}
	return multiple
}

func MaxMultiple(opts *protocol.DeskOptions, onHand, pongkong Indexes) (multiple, index int) {
