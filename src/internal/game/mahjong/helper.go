...
func isJiaxin(ctx *Context, onHand Indexes) bool {
	...
	//默认胡5条
	willRomoveTiles := Indexes{4, 5, 6}
	if index == 15 {
		willRomoveTiles = Indexes{14, 15, 16}
	} else if index == 25 {
		willRomoveTiles = Indexes{24, 25, 26}
	}
	//卡5星判断规则
	//胡的牌必须是5条,5筒
	//移除4,5,6 OR 14,15,16 OR 24,25,26后仍然可以胡牌
