...
func logRequest(ctx context.Context, r *http.Request) (context.Context, error) {
	if uri := r.RequestURI; uri != "/ping" {
		logger.Debugf("Mothod=%s,RemoteAddr=%s URL=%s", r.Method, r.RemoteAddr, uri)
	}
	return ctx, nil
}
...
func Startup() {
	...
	var (
		...
		cert      = viper.GetString("webserver.certficates.cert")
		key       = viper.GetString("webserver.certficates.key")
		enableSSL = viper.GetBool("webserver.enable_ssl")
	)
	logger.Infof("Web service addr: %s(enable_ssl: %v)", addr, enableSSL)
	go func() {
		//http service
		mux := startupService()
		if enableSSL {
