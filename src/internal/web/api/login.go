...
func MakeLoginService() http.Handler {
	...
	logger.Infof("是否开启游客登录: %t, 渠道列表: %v", enableGuest, guestChannels)
	...
	logger.Debugf("version information: %+v", config)
	...
	router.Handle("/v1/uer/login/guest", nex.Handler(guestLoginHandler)).Methods("POST")
...
func thirdUserLoginHandler(r *http.Request, data *protocol.ThirdUserLoginRequest) (*protocol.LoginResponse, error) {
	...
	if err != nil && err != errutil.ErrThirdAccountNotFound { //这里是sql出错
		logger.Error(err)
		return nil, err
	}
	...
	} else { //err == errutil.ErrThirdAccountNotFound
		u = &model.User{Status: db.StatusNormal, IsOnline: db.UserOffline}
		u.Role = db.RoleTypeThird
		u.Coin = defaultCoin
...
func guestLoginHandler(r *http.Request, data *protocol.LoginRequest) (*protocol.LoginResponse, error) {
	...
	logger.Infof("游客登录IMEI: %s", data.Device.IMEI)
	...
	if err != nil {
		...
		//注册记录
		db.RegisterUserLog(user, data.Device, data.AppID, data.ChannelID, protocol.RegTypeThird)
	}
...
var (
	...
	acceptGuest = &queryResponse{Guest: true}
)
