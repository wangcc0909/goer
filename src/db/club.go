...
func ApplyClub(uid, clubId int64) error {
	...
	if ok {
		if uc.Status == model.UserClubStatusAgree {
			return errors.New("你已经加入该俱乐部,无需申请")
		}
		if uc.Status == model.UserClubStatusApply {
			return errors.New("你已经申请加入该俱乐部,等待部长同意")
		}
	}
...
func ClubList(uid int64) ([]model.Club, error) {
	...
	var list []model.UserClub
	if err := database.Find(&list, bean); err != nil { //这里是查找出错
		return nil, err
	}
	if len(list) < 1 { //这里是没找到
	...
	var ret []model.Club
	err := database.In("club_id", ids).Find(&ret)
	if err != nil {
		return nil, err
	}
	return ret, nil
